...
What the transformer emits per construct: **source → compiled Luau + injected registration call**. For how the runtime then *uses* these registrations see [Runtime lifecycle](20-runtime-lifecycle.md).
...
Hook wiring (archetype tracking, `world:added/removed/changed`) is deferred to finalize — see [Runtime lifecycle § Monitors](20-runtime-lifecycle.md#monitors).
...
`rovy.__traitQuery` resolves at finalize to N jecs sub-queries (one per registered implementer), wrapped in a `TraitQueryHandle`. See [Runtime lifecycle § Traits](20-runtime-lifecycle.md#traits).
...
Runtime applies the tick check during iteration — see [Runtime lifecycle § Change detection](20-runtime-lifecycle.md#change-detection).
...
The transformer's job ends at injecting these calls. What happens when they run, and when `app.start()` finalizes them, is [Runtime lifecycle](20-runtime-lifecycle.md).
