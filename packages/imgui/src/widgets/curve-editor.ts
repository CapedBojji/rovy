...
function normalizePathOptions(options: CurvePathOptions): StoredCurvePath | undefined {
	...
	if (id === "") {
		warn("[rovy-imgui] curvePath requires a non-empty id");
		return undefined;
	}
	if (options.commands.size() < 2 || options.commands[0].kind !== "M") {
		warn(`[rovy-imgui] curvePath "${id}" must start with M and contain at least two editable points`);
		return undefined;
	}
	...
	if (editablePointCount < 2) {
		warn(`[rovy-imgui] curvePath "${id}" must contain at least two editable points`);
		return undefined;
	}
...
function reconcilePathStore(store: CurveEditorPathStore, registrations: ReadonlyArray<CurvePathOptions>): boolean {
	...
	for (const registration of registrations) {
		...
		if (seen.has(normalized.id)) {
			warn(`[rovy-imgui] duplicate curvePath id "${normalized.id}" ignored`);
			continue;
		}
...
}, "@rovy/imgui/curveEditorContextHeader");
...
function createEditableImageForGraph(refs: CurveEditorRefs, size: Vector2): void {
	...
	if (!serviceOk) {
		refs.errorLabel.Text = `[rovy-imgui] curveEditor AssetService unavailable: ${tostring(serviceOrError)}`;
	...
	if (!imageOk) {
		refs.errorLabel.Text = `[rovy-imgui] curveEditor image create failed: ${tostring(imageOrError)}`;
		refs.errorLabel.Visible = true;
		refs.imageLabel.Visible = false;
		refs.imageCreateFailedSize = size;
		return;
	...
	if (!contentOk) {
		refs.errorLabel.Text = `[rovy-imgui] curveEditor image display failed: ${tostring(contentError)}`;
		refs.errorLabel.Visible = true;
		refs.imageLabel.Visible = false;
		return;
	}
...
function redrawIfNeeded(refs: CurveEditorRefs): void {
	...
	if (!ok) {
		refs.errorLabel.Text = `[rovy-imgui] curveEditor draw failed: ${tostring(err)}`;
		refs.errorLabel.Visible = true;
	}
}
...
export const curvePath = widget((options: CurvePathOptions): CurvePathHandle => {
	...
	if (pathScope === undefined) error("[rovy-imgui] curvePath must be used inside curveEditor", 2);
	return pathScope.register(options);
}, "@rovy/imgui/curvePath");
