...
How the runtime finds, finalizes, and uses every registered construct. Pairs with [Compiled output](/runtime/compiled-output.md), which covers what the transformer emits.
...
3. **Run** — `app.runSchedule(X, dt?)` per frame: resolve params, iterate queries, dispatch, flush.
...
### Schedule time and `ScheduleContext`

Roblox frame loops are the source of `dt`. `app.runSchedule(schedule, dt)` is the boundary where wall-clock frame time enters Rovy.

`app.start()` auto-registers a built-in `ScheduleContext` resource, but it is still just a normal injected param. Systems only receive it when they explicitly ask for it through resource injection, instead of sampling `os.clock()` independently:

```ts
import { ScheduleContext } from "@rovy/core";

run(ctx: ScheduleContext) {
    print(ctx.schedule, ctx.dt, ctx.rawDt, ctx.elapsed, ctx.frame);
}
```

- `dt` is clamped to `>= 0`.
- `rawDt` preserves the exact caller-provided value.
- `elapsed` and `frame` are global counters across all schedule runs after `app.start()`.
- Nested `commands.runSchedule(...)` / `world.runSchedule(...)` calls restore the outer schedule's `schedule` + `dt` view after the inner run finishes.

Use `ScheduleContext.dt` for per-frame delta inside systems. Do not call `os.clock()` independently for schedule `dt`, or systems in the same schedule can drift and disagree.
...
per frame: app.runSchedule(Update, dt)
