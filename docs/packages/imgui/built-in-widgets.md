...
All built-ins are plain functions exported from `@rovy/imgui`. Import named helpers or use the default `RovyUi` object.

```ts
import { button, label, window } from "@rovy/imgui";
