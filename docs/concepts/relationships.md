...
Relationship lifecycle (enter, exit, change) uses `@monitor` with `Pair<R>` in the match query. Observers are event-only — see [Observers](/concepts/observers.md).
...
All three methods (`onEnter`, `onExit`, `onChange`) are optional — implement only what you need. For full monitor options see [Monitors](/concepts/monitors.md).
...
`app.start()` allocates jecs pair IDs and applies cleanup policies. See [Compiled output](/runtime/compiled-output.md).
