...
Sets are classes extending `SystemSet`. See [Systems and injection](/concepts/systems-and-injection.md#sets).
...
`rovy.loadPaths(...)` makes these run; `app.start()` builds the schedule objects and fires `runOnStart` ones. See [Compiled output](/runtime/compiled-output.md).
