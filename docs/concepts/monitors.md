...
> **Compile-time.** `@monitor` decorator with `match: query<...>()` macro. Transformer reads the query type params, builds an archetype-based monitor, and validates `onEnter`/`onExit`/`onChange` param types match the query terms. All component lifecycle reactions go through monitors — [observers](/concepts/observers.md) are event-only.
...
- [Observers](/concepts/observers.md) — event-only reactions
