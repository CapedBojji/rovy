...
function createEditableImage(refs: EditableImageRefs, size: Vector2): void {
	if (!isValidImageSize(size)) {
		refs.ready = false;
		setError(refs, "[rovy-imgui] editableImage size must be integer pixels from 1..1024");
		return;
	}
	...
	if (!serviceOk) {
		refs.ready = false;
		setError(refs, `[rovy-imgui] editableImage AssetService unavailable: ${tostring(serviceOrError)}`);
		return;
	}
	...
	if (!imageOk) {
		refs.ready = false;
		setError(refs, `[rovy-imgui] editableImage create failed: ${tostring(imageOrError)}`);
		return;
	}
	...
	if (!contentOk) {
		refs.ready = false;
		setError(refs, `[rovy-imgui] editableImage display failed: ${tostring(contentError)}`);
		return;
	}
...
export const editableImage = widget((options: EditableImageOptions): EditableImageHandle => {
	...
	if (refs.size !== undefined && (refs.size.X !== options.size.X || refs.size.Y !== options.size.Y)) {
		setError(refs, "[rovy-imgui] editableImage size cannot change after creation");
	}

	return {
		draw: (pixels: buffer): boolean => {
			if (refs.ready !== true || refs.editableImage === undefined || refs.size === undefined) {
				setError(refs, refs.lastError ?? "[rovy-imgui] editableImage unavailable");
				return false;
			}
			...
			if (buffer.len(pixels) !== expectedLength) {
				setError(
					refs,
					`[rovy-imgui] editableImage buffer length ${buffer.len(pixels)} does not match ${expectedLength}`,
				);
				return false;
			}
			...
			if (!ok) {
				setError(refs, `[rovy-imgui] editableImage draw failed: ${tostring(err)}`);
				return false;
			}
