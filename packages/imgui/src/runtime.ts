...
function rootNode(): Node {
	...
	assert(root !== undefined, "[rovy-imgui] pointer helpers require an active frame");
	return root;
}
...
export function currentFrame(): StackFrame {
	...
	assert(frame !== undefined, "[rovy-imgui] UI helpers can only run inside RovyUi.start/scope/widget");
	return frame;
}
...
function invokeNoYield<T>(fn: () => T): T {
	...
	assert(coroutine.status(thread) === "dead", "[rovy-imgui] widget handler yielded; yielding is not allowed");
	if (!success) error(`[rovy-imgui] ${tostring(result)}`, 2);
	return result as T;
}
...
export function scope<T>(fn: () => T): T {
	error("[rovy-imgui] scope(...) must be lowered by rovy-transformer");
}
...
export function useState<T>(_initialValue: T | (() => T)): [T, StateSetter<T>] {
	error("[rovy-imgui] useState(...) must be lowered by rovy-transformer");
}
...
export function useEffect(_callback: () => void | EffectCleanup, ..._dependencies: Array<unknown>): void {
	error("[rovy-imgui] useEffect(...) must be lowered by rovy-transformer");
}
...
export function __useInstance<T extends object = Record<string, Instance>>(
	...
	assert(parentFrame !== undefined, "[rovy-imgui] useInstance requires a parent root instance");
...
export function useInstance<T extends object = Record<string, Instance>>(
	...
	error("[rovy-imgui] useInstance(...) must be lowered by rovy-transformer");
}
...
export function setEventCallback(callback: EventCallback): void {
	const root = stack[0];
	assert(root !== undefined, "[rovy-imgui] setEventCallback requires an active frame");
