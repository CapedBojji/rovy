...
Inside custom widgets, use `useStyle()` unless you are using the leading `style: Style` authoring sugar described in [Custom Widgets](/packages/imgui/custom-widgets).
...
The `Style` interface lives in `packages/imgui/src/style.ts`. It includes:
