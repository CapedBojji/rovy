# `examples/tower-defense-game` Rovy-First Example Plan

## Summary
- Add new example package at `examples/tower-defense-game` as fuller showcase than current tiny example.
...
## Package Setup
- Create new workspace package `@rovy/example-tower-defense-game` under `examples/tower-defense-game`.
- Add `package.json`, `tsconfig.json`, `default.project.json`, build/test scripts, and output place `build/rovy-tower-defense-game.rbxl`.
...
- `test/harness/runtime.luau`
  - current example harness clone, extended only if needed by tower defense smoke paths
...
- `build:place` produces non-empty `build/rovy-tower-defense-game.rbxl`.
